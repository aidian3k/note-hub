...
public class AppBenchmark extends AbstractBenchmark {
  ...
  public void initUser() {
    ...
    userRegistrationRequest = UserRegistrationRequest.builder()
        .firstName("Jan").lastName("Kowal").email("123@wp.pl")
        .password("1slkdjnlsdm").confirmationPassword("1slkdjnlsdm").build();

    when(currentUserService.getCurrentUserEmbeddedId()).thenReturn("user@example");
    when(userRegistrationService.registerKeycloakUser(userRegistrationRequest)).thenReturn(201);
    when(userLoginService.handleUserLogin(userLoginRequest)).thenReturn(UserLoginResponse.builder().access_token("accessToken").expires_in("123132").refresh_expires_in(123123).refresh_token("refreshToken").build());
    when(userRefreshTokenManagementService.handleRefreshToken(REFRESH_TOKEN)).thenReturn(UserLoginResponse.builder().access_token("accessToken").expires_in("123132").refresh_expires_in(123123).refresh_token("refreshToken").build());
  }
